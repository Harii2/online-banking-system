...
class GetTransactionHistoryInteractor:
    ...
    def get_transaction_history(self, account_id: int, presenter: PresenterImplementation):
        try:
            self.storage.validate_account_id(account_id)
        except InvalidAccountId:
            presenter.raise_invalid_account_id()
            return
        transaction_history_response_dto = self.storage.get_transaction_history(account_id)
